...
public class UserService : IUserService
{
    ...
    public UserService(ILogger<IUserService> log, CoreContext ctx, IOptions<TemplateSetting> tpstg, IOptions<PathSetting> pthstg, UserMap mapuser)
    {
        logger = log;
        db = ctx;
    ...
    {
        var user = db.CoreUsers.FirstOrDefault(e => e.UserId == id || e.UserName == id);
        if (user != null)
        {
    ...
    public bool Remove(string ids)
    {
        try
        {
            if (!string.IsNullOrEmpty(ids))
            {
                ...
                var removeUsers = db.CoreUsers.Where(e => idlist.Contains(e.Id)).ToList();
    ...
    public bool Save(IUser user)
    {
        try
        {
            var founduser = db.CoreUsers.FirstOrDefault(e => e.UserId == user.UserId);
            var map = user as UserModel;
            ...
            if (map != null)
            {
                ...
                if (isnew)
                    founduser = new CoreUser();
                if (founduser != null)
                {
                    founduser= UserMap.ToModel(map, founduser);
                    
                    if (isnew)
                    {
                        db.CoreUsers.Add(founduser);
    ...
    public RM.LinkResult Export(EM.MantineTableProps query)
    {
        ...
        {
            query.Start = 0;
            query.Size = db.CoreUsers.Count();
            var dataresult = List(query).data.ToList();
            var datatable = ExcelHelper.ListToDataTable(dataresult.Select(e => UserMap.FromModel((CoreUser)e)).ToList(), true);
    ...
    public RM.UserListResult List(EM.MantineTableProps query)
    {
        try
        {
            ...
            foreach (var filter in query.Filtering)
            {
                ...
                nv = filterIds switch
                {
                    var x when x.Contains(nameof(CoreUser.UserId)) => nv.AddQueryParam(db.CoreUsers, x => x.UserId, value),
                    var x when x.Contains(nameof(CoreUser.UserName)) => nv.AddQueryParam(db.CoreUsers, x => x.UserName, value),
                    var x when x.Contains(nameof(CoreUser.loginedAt)) =>

                    nv.AddQueryParam(db.CoreUsers, x => x.loginedAt, valueobjs[0]?.ToString(), Op.greaterThanOrEqual)
                    .AddQueryParam(db.CoreUsers, x => x.loginedAt, valueobjs[1]?.ToString(), Op.lessThanOrEqual)
                     ,
                    var x when x.Contains(nameof(CoreUser.IsControlAdmin)) =>
                    nv.AddQueryParam(db.CoreUsers, x => x.IsAdmin, value, Op.equal)
                    .AddQueryParam(db.CoreUsers, x => x.AdminScope, bool.Parse(value ?? "False") ? "Full" : null, Op.equal),
                    _ => nv,
            ...
            if (!allRecords)
            {
                nv.AddQueryParam(db.CoreUsers, x => x.Disabled, false.ToString(), Op.equal);
            }

            var result = db.GetSearch<CoreUser>(nv);
    ...
    public bool ChangePassword(string username, string password, string byusername, string oldpassword = null)
    {
        try
        {
            var user = db.CoreUsers.FirstOrDefault(e => e.UserName == username || e.UserId == username);
            if (user == null)
            {
                return false;
            }
